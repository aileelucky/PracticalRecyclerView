 ...
 * @author gujiajia
 */
public class PRProgressWheel extends View {
    ...
    public PRProgressWheel(Context context, AttributeSet attrs) {
        super(context, attrs);

        parseAttributes(context.obtainStyledAttributes(attrs,
                R.styleable.PR_ProgressWheel));
    }

    /**
     * The constructor for the ProgressWheel
     */
    public PRProgressWheel(Context context) {
        super(context);
    }
    ...
    private void parseAttributes(TypedArray a) {
        ...
        circleRadius = (int) a.getDimension(R.styleable.PR_ProgressWheel_pr_matProg_circleRadius, circleRadius);

        fillRadius = a.getBoolean(R.styleable.PR_ProgressWheel_pr_matProg_fillRadius, false);

        barWidth = (int) a.getDimension(R.styleable.PR_ProgressWheel_pr_matProg_barWidth, barWidth);

        rimWidth = (int) a.getDimension(R.styleable.PR_ProgressWheel_pr_matProg_rimWidth, rimWidth);

        float baseSpinSpeed = a.getFloat(R.styleable.PR_ProgressWheel_pr_matProg_spinSpeed, spinSpeed / 360.0f);
        spinSpeed = baseSpinSpeed * 360;

        barSpinCycleTime = a.getInt(R.styleable.PR_ProgressWheel_pr_matProg_barSpinCycleTime, (int) barSpinCycleTime);

        barColor = a.getColor(R.styleable.PR_ProgressWheel_pr_matProg_barColor, barColor);

        rimColor = a.getColor(R.styleable.PR_ProgressWheel_pr_matProg_rimColor, rimColor);

        linearProgress = a.getBoolean(R.styleable.PR_ProgressWheel_pr_matProg_linearProgress, false);

        if (a.getBoolean(R.styleable.PR_ProgressWheel_pr_matProg_progressIndeterminate, false)) {
            spin();
        }
        a.recycle();
    }
